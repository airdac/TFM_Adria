...
```{r plot comparison}
MDS.benchmark.data <- read.csv(file_name, header = TRUE, stringsAsFactors = T)
MDS.benchmark.data$d <- as.factor(MDS.benchmark.data$d)
summary(MDS.benchmark.data)

# Summarize data for each library and d combination
summary_data <- MDS.benchmark.data %>%
  filter(d==8) %>%
...
# Summarize the data to calculate average and standard error of time for each n_obs and d
summary_data <- MDS.benchmark.data %>%
...
R crashes for when d == 8 and n_obs == 56234.