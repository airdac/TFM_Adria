...
def divide_conquer_isomap(x, l, c_points, r, n_cores, n_neighbors):
    """
    ...
    if n_row_x <= l:
        ...
        # Process remaining partitions in parallel.
        with ProcessPoolExecutor(max_workers=n_cores) as executor:
            futures = [executor.submit(main_divide_conquer_isomap, x[idx, :], x_sample_1, r, isomap_sample_1, n_neighbors)
...
if __name__ == "__main__":
    ...
    X, color = make_swiss_roll(n_samples=3000, noise=0.1, random_state=42)

    print("First few points of high-dimensional data:")
    print(X[:6])

    # Apply divide and conquer isomap and measure parallel vs single-core runtimes
    start_time = time.time()
    result = divide_conquer_isomap(
        x=X, l=100, c_points=4, r=2, n_cores=os.cpu_count(), n_neighbors=10)
    parallel_runtime = time.time() - start_time

    start_time = time.time()
    result2 = divide_conquer_isomap(
        x=X, l=100, c_points=4, r=2, n_cores=1, n_neighbors=10)
    single_core_runtime = time.time() - start_time

    print(f"Parallel runtime: {parallel_runtime:.2f} seconds")
    print(f"Single-core runtime: {single_core_runtime:.2f} seconds")

    # Print results
    print("First few points:")
    print(result['points'][:6])
    print("\nEigenvalues:")
    print(result['eigen'])
    ...
    ax2.scatter(result['points'][:, 0], result['points']
                [:, 1], c=color, cmap=plt.cm.Spectral)
    ax2.set_title("D&C Isomap Embedding")

    plt.tight_layout()
    plt.savefig(os.path.join('figures', 'isomap_d_and_c.png'))
